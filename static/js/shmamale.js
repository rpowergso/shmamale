...
function cardAtPoint(clientX, clientY) {
    ...
    if (hit?.closest?.(".held-slot, .nameplate, .game-rail, .overlay-card")) return null;
    ...
    for (const card of document.querySelectorAll(".board-card:not(.anim-hidden):not(.ability-picked-up)")) {
        ...
        const polygon = [[-1,-1],[1,-1],[1,1],[-1,1]].map(([x,y]) => {
            ...
            const projected = projectWorldPoint(
                center.x + frame.tangent.x * dx + frame.outward.x * dy,
        ...
        <section class="${classes.join(" ")}" style="${style}" data-sid="${sid}">
            <div class="seat-scale">
                <div class="seat-board">
                    <div class="card-grid cols-${cols}" data-grid="${sid}" style="grid-template-columns:repeat(${cols}, minmax(0, 1fr))">${cards}</div>
                </div>
            </div>
        </section>
    `;
...
function renderBurnShowdown() {
    ...
    for (const result of history) {
        ...
        if (!group) {
            group = { ...result, attempts: [] };
            groups.push(group);
        }
        group.attempts.push(...result.attempts);
    }
    lastBurnShowdownId = state.burn_showdown?.id || 0;
    els.burnShowdown.innerHTML = `<h2>Burn order</h2><p class="burn-help">From pile placement · gap from previous attempt</p>${groups.reverse().map(burnShowdownHtml).join("") || '<p class="burn-empty">Burn attempts appear here.</p>'}`;
}
...
function flyCardOnPlane({
    ...
    return new Promise((resolve) => {
        ...
        function frame(now) {
            ...
            const cardScale = (start.scale || 1) + ((end.scale || 1) - (start.scale || 1)) * e;
            setPose(x, y, yaw, tilt, lift, cardScale);
            if (t < 1) {
...
function burnCard(ownerSid, index) {
    ...
    if (!canAttemptBoardBurn(inspection) || localBurnAttempt || !slot || slot.empty
        || isBurnBlocked(ownerSid, index) || canOpeningPeek(ownerSid, index, slot)) return;
