...
class BurnRaceTests(unittest.TestCase):
    ...
    def test_discard_remains_burnable_after_an_all_miss_showdown(self):
        ...
        self.assertEqual(history[0]["attempts"][0]["result"], "miss")
        self.assertEqual(history[1]["attempts"][0]["result"], "winner")
        self.assertEqual(history[0]["placed_at"], history[1]["placed_at"])
        self.assertEqual(history[1]["attempts"][0]["time_ms"], 1600)

    def test_wrong_attempt_before_winner_uses_discard_origin_and_first_attempt_gap(self):
        game, discard = self.make_race_game()
        game["players"]["bot1"]["board"][2] = make_slot(card("8", "clubs"))
        started = game["burn_window_started_at"]
        ...
        self.assertAlmostEqual(game["burn_contests"][discard["id"]]["resolve_at"], started + 0.62)
        finalize_burn_contest(game, discard["id"])
...
class LiveBurnShowdownTests(unittest.TestCase):
    def test_peek_burns_include_successes_and_misses_in_history(self):
        for rank, result in [("7", "winner"), ("9", "miss")]:
            with self.subTest(result=result):
                ...
                try:
                    ...
                    game["held_peek"] = {"sid": sid, "owner_sid": sid, "index": 0, "card": card(rank)}
                    game["pending_ability"] = {"sid": sid, "type": "peek_own", "stage": "holding"}
                    discard = card("7", "hearts")
                    game["discard_pile"] = [discard]
                    ...
                    self.assertEqual(history[0]["attempts"][0]["penalty"], result == "miss")
                finally:
                    client.disconnect()
                    rooms.pop(room, None)
