...
from multiplayer import emit_state, rooms


@app.post("/qa/reset")
def reset_visual_room():
    rooms.pop("VISUAL", None)
    return jsonify(ok=True)


@app.post("/qa/setup")
def setup_visual_room():
    ...
    game.update(
        status="playing", phase="choose", turn_index=1, pending_draw=None,
        ...
        first_caller_sid=None, final_turns_remaining=[], burnt_slots=[],
        burn_blockers=[], failed_burn_reveals=[], active_burn_contest_id=None,
        burn_contests={},
    )
    game["settings"].update(
        grid_rows=rows, grid_cols=cols, grid_peek_modes=["none"] * (rows * cols),
    )
    ...
    for i, owner in enumerate(people):
        ...
        player.update(
            username=names[i], called=False, protected=False,
            first_turn_started=True, opening_peeked=[],
        )
        player["board"] = [
            make_slot(make_card(
                ["7", "9", "2", "K"][j % 4],
                suit="hearts" if j % 2 else "clubs",
                deck_number=i * 20 + j + 1,
            ))
            for j in range(rows * cols)
        ]
    game["discard_pile"] = [make_card("7", suit="hearts", deck_number=200)]
    game["draw_pile"] = [
        make_card(str(2 + i % 8), suit="diamonds", deck_number=300 + i)
        for i in range(40)
    ]
    ...
    if data.get("held"):
        game["pending_draw"] = {
            "sid": people[1], "card": game["draw_pile"].pop(), "source": "draw",
        }
        game["phase"] = "holding"
    if data.get("round_over"):
        game.update(status="round_over", phase="round_over")
        game["round_results"] = {
            "raw_scores": {owner: 12 for owner in people},
            "round_scores": {owner: 12 for owner in people},
            "eliminated": [], "next_start_sid": sid,
        }
